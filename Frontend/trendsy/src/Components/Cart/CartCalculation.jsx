...
const [check, setCheck] = useState(false);
  ...
  const Calculate = () => {
    ...
    const coupenDiscount=Math.floor((cal.total-cal.discount)*(cal.coupen/100))
    console.log(cal.total)
    cal.total_Amount =
      cal.total - cal.discount-coupenDiscount + cartTotals.fee + cartTotals.social-cal.loginDic;
    setCartTotals(cal);
  };
  ...
  const handleLoginDisc=()=>{
   ...
   setCartTotals( newCartTotals);
    ...
    toast({
      position: "top",
      title: cartTotals.loginDic? "Discount applied already":"You are not new user",
      description:cartTotals.loginDic?"This Discount is applicable only once": "This Discount is applicable only for new user ",
      status: "info",
      duration: 3000,
      isClosable: true,
    });
  ...
  const handleCoupen=()=>{
    ...
    const timeId = setTimeout(async () => {
      ...
      if (Object.keys(CoupenData).includes(coupen)) {
        ...
        newCartTotal.total_Amount=newCartTotal.total-Math.floor((newCartTotal.total-newCartTotal.discount)*(CoupenData[coupen]/100))
...
setSupportDiv(supportID)
  ...
  return (
    ...
    >
      <VStack align="start" fontSize={["10px", "xs", null, "sm"]}>
        ...
        <HStack w="full">
          ...
          <HStack justify="space-between" w="full">
            ...
            <Button size={"sm"} variant="outline" color={cartTotals.coupen?"green":"#ff3f6c"} onClick={onOpen}>
            ...
            <ModalBody pb={6}>
              <HStack>
                ...
                <Button
                  ...
                  variant={checkCoupen.color != "blackAlpha" ? "solid" : "outline"}
      ...
      {cartTotals.total&&  <HStack>
         <Button size={["xs","sm"]}variant="unstyled" color= {cartTotals.loginDic?"green.500": "#ff3f6c"} onClick={handleLoginDisc}>
            {cartTotals.loginDic?"Applied": "User Check"}
          </Button>
          <Text fontSize={["xs","sm"]}> to get upto ₹ 300 OFF on first order</Text>
        </HStack>}
      </VStack>
      <Text as="b" color="blackAlpha.700">
      ...
      <Box w="full">
        ...
        >
          ...
          <HStack justify={"space-between"} w="full">
            ...
            {cartTotals.coupen?<Text color="green.300"> {cartTotals.coupen} % OFF</Text>:<Button size="sm" variant="unstyled" color="#ff3f6c" h="-5px" onClick={onOpen}>
              Apply Coupon
            </Button>}
          </HStack>
          {cartTotals.loginDic&&cartTotals.total&&<HStack justify={"space-between"} w="full">
